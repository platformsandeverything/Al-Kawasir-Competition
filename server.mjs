...
http.createServer(async(req,res)=>{
  ...
  if(url.pathname==='/api/game'&&req.method==='POST'){
    try{let raw='';for await(const chunk of req)raw+=chunk;const incoming=JSON.parse(raw);const known=new Map((gameState?.teams||[]).map(t=>[t.id,t]));for(const team of incoming.teams||[])known.set(team.id,{...known.get(team.id),...team});gameState={...gameState,...incoming,teams:[...known.values()],updated:Date.now()};return send(res,200,gameState)}catch{return send(res,400,{error:'بيانات غير صالحة'})}
  }
